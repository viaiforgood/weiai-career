...
import { createRemoteJWKSet, importPKCS8 as importPKCS83, jwtVerify, customFetch as customFetch3, jwksCache, decodeJwt } from "jose";
...
import { SignJWT as SignJWT2, importPKCS8 as importPKCS82 } from "jose";
...
async function buildClientAuthBody3(options, clientId, domain) {
  ...
  if (options.clientAssertionSigningKey) {
    ...
    const privateKey = options.clientAssertionSigningKey instanceof CryptoKey ? options.clientAssertionSigningKey : await importPKCS82(options.clientAssertionSigningKey, alg);
    const clientAssertion = await new SignJWT2({}).setProtectedHeader({ alg }).setIssuer(clientId).setSubject(clientId).setAudience(`https://${domain}/`).setJti(crypto.randomUUID()).setIssuedAt().setExpirationTime(`${CLIENT_ASSERTION_EXPIRY_SECONDS2}s`).sign(privateKey);
    return {
      client_assertion: clientAssertion,
    ...
    if (!this.#clientAuthPromise) {
      this.#clientAuthPromise = (async () => {
        ...
        if (clientPrivateKey && !(clientPrivateKey instanceof CryptoKey)) {
          clientPrivateKey = await importPKCS83(
            clientPrivateKey,
